...
echo "Building..."
if ! docker build -t supporttools/k3s-janitor:${DRONE_BUILD_NUMBER} --cache-from supporttools/k3s-janitor:latest --build-arg REACT_APP_URL="/api" --build-arg GIT_COMMIT=${GIT_COMMIT} -f Dockerfile .
then
